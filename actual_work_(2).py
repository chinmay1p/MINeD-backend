...
Original file is located at
    https://colab.research.google.com/drive/1j69R4mk4bty7AVUQ6dvBBtbkJTiYNUUj
"""
...
import google.generativeai as genai  # Assuming you have this set up
...
def extract_company_info(pdf_path, pages=2):
    """Extracts Company Name and BSE Code from a financial report PDF."""
    # Step 1: Extract company name from the PDF
    company_name = extract_company_name_from_pdf(pdf_path, pages)
...
# def extract_company_info(pdf_path, pages=2):
#     """Extracts Company Name and BSE Code from a financial report PDF using Google Gemini API."""
...
#     try:
#         model = genai.GenerativeModel("gemini-pro")
#         company_name_response = model.generate_content(company_name_prompt)
#         company_name = company_name_response.text.strip() if company_name_response.text else "Company Name Not Available"
#         print(company_name)
#         # Step 3: Extract the BSE code
#         bse_code_prompt = f"""
#         Extract the 6-digit BSE code of the company from the bse official website or the web.
#         The company name is: {company_name}.
#         If the BSE code is not available, search it from the web.
#         Only return the BSE code or "Not Available".

#         Report Content:
#         {text}
#         """

#         bse_code_response = model.generate_content(bse_code_prompt)
#         bse_code = bse_code_response.text.strip() if bse_code_response.text else "Not Available"
...
if __name__ == "__main__":
    ...
    pdf_path = "/content/c389663f-3f7a-4c58-84d8-5ba2debfd886.pdf"  # Replace with your PDF path
    ...
    # Step 2: Detect report types (Consolidated, Standalone, or Both)
    report_type = detect_report_types(pdf_text)
    print(f"Detected Report Type: {report_type}")

    # Step 3: Fetch tables from Screener.in based on detected report types

    if report_type.lower() == "both":
        # Fetch both consolidated and standalone tables
        consolidated_table = scrape_screener_quarterly_report_table(company_info, "consolidated")
        standalone_table = scrape_screener_quarterly_report_table(company_info, "standalone")

        if consolidated_table:
            display_table(consolidated_table, "Consolidated Quarterly Report Table")
        else:
            print("Could not retrieve consolidated quarterly report table.")

        if standalone_table:
            display_table(standalone_table, "Standalone Quarterly Report Table")
        else:
            print("Could not retrieve standalone quarterly report table.")

    elif report_type.lower() in ["consolidated", "standalone"]:
        # Fetch the corresponding table
        table_data = scrape_screener_quarterly_report_table(company_info, report_type.lower())
        if table_data:
            display_table(table_data, f"{report_type.capitalize()} Quarterly Report Table")
        else:
            print(f"Could not retrieve {report_type} quarterly report table.")
    else:
        print("Unable to detect report type. Please check the PDF content.")
